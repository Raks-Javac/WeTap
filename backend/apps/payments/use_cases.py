...
def enforce_kyc(user, amount: Decimal):
    profile = KYCRepository.by_user(user)
    if not profile:
        return
...
def fund_wallet(user, amount, idempotency_key: str = ""):
    ...
    tx = TransactionRepository.create(user=user, reference=reference, type=Transaction.TYPE_FUND, status=TX_PENDING, amount=amount)
    result = get_payment_processor().charge_card({"reference": reference, "amount": float(amount)})
...
def execute_nfc_payment(user, session, card_id: str, method: str, amount, idempotency_key: str, pin: str = ""):
    ...
    amount = to_decimal(amount)
    enforce_kyc(user, amount)

    card = CardRepository.active_for_user(card_id=card_id, user=user)
    if not card:
    ...
    result = get_payment_processor().charge_card({"reference": reference, "amount": float(amount), "card_token": card.token_encrypted})
