...
class OTPUseCase:
    ...
    def request_otp(email: str, ip_address: str):
        ...
        cache.set(cache_key, make_password(otp), timeout=OTPUseCase.TTL_SECONDS)

        OTPRepository.create_token(
            email=email,
            otp_hash=make_password(otp),
            expires_at=timezone.now() + timedelta(seconds=OTPUseCase.TTL_SECONDS),
        )
        return {"email": email, "otp": otp, "ttl": OTPUseCase.TTL_SECONDS}
    ...
    def verify_otp(email: str, otp: str):
        ...
        if not hashed:
            fallback = OTPRepository.latest_unused_token(email)
        ...
        return UserRepository.get_or_create_by_email(email)
